...
namespace LB1
{
    public partial class Organizer_window : Form
    {
        ...
        private void Add_Click(object sender, EventArgs e)
        {
            ...
            {
                DataEvents.Rows.Add();
                if (Temp.type == "Памятка") { DataEvents[0, DataEvents.Rows.Count - 1].Value = new Bitmap("rrr.png"); }
                if (Temp.type == "Встреча") { DataEvents[0, DataEvents.Rows.Count - 1].Value = new Bitmap("ttt.png"); }
                if (Temp.type == "Задание") { DataEvents[0, DataEvents.Rows.Count - 1].Value = new Bitmap("qqq.png"); }
        ...
        {
        metca1:
            string stroka = Interaction.InputBox("Поиск", "Введите событие", "Текст");
            if (stroka == "") 
            {
                return; 
            } 
        ...
        private void Organizer_window_Load(object sender, EventArgs e)
        {
            ...
            add_Button.ForeColor = System.Drawing.Color.Black;
        ...
        private void openfile()
        {
            if (System.IO.File.Exists(writePath))
            {
                using (StreamReader sr = new StreamReader(writePath, System.Text.Encoding.Default))
                {
                    ...
                    for (int i = 0; i < DataEvents.Rows.Count; i++)
                    {
                        if (DataEvents.Rows[i].Cells[4].Value.ToString() == "Памятка") { DataEvents.Rows[i].Cells[0].Value = new Bitmap("rrr.png"); }
                        if (DataEvents.Rows[i].Cells[4].Value.ToString() == "Встреча") { DataEvents.Rows[i].Cells[0].Value = new Bitmap("ttt.png"); }
                        if (DataEvents.Rows[i].Cells[4].Value.ToString() == "Задание") { DataEvents.Rows[i].Cells[0].Value = new Bitmap("qqq.png"); }
                    }
                }
            }
            flag1 = 0;
